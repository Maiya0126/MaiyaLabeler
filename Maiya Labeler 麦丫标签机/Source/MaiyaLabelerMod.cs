...
namespace MaiyaLabeler
{
    ...
    public static class MaiyaLabelerBootstrapper
    {
        ...
        public static readonly Texture2D IconRoom = ContentFinder<Texture2D>.Get("UI/Designators/BuildRoofArea", false) ?? ContentFinder<Texture2D>.Get("UI/Designators/HomeAreaOn", false);
        public static readonly Texture2D IconGrow = ContentFinder<Texture2D>.Get("UI/Designators/ZoneCreate_Growing", false) ?? ContentFinder<Texture2D>.Get("UI/Designators/Plan", false);
        public static readonly Texture2D IconStock = ContentFinder<Texture2D>.Get("UI/Designators/ZoneCreate_Stockpile", false) ?? ContentFinder<Texture2D>.Get("UI/Designators/Plan", false);
    ...
    public static class Patch_Zone_GetInspectString
    {
        static void Postfix(Verse.Zone __instance, ref string __result)
        {
            ...
            if (data != null && !string.IsNullOrEmpty(data.customName))
            {
                ...
                // 使用翻译Key: MaiyaLabeler_NamePrefix
    ...
    public static class Patch_PlaySettings
    {
        static void Postfix(WidgetRow row, bool worldView)
        {
            ...
            // 使用翻译Key: MaiyaLabeler_ToggleIconDesc
            row.ToggleableIcon(
                ref isActive,
                MaiyaLabelerBootstrapper.IconTool,
                "MaiyaLabeler_ToggleIconDesc".Translate(),
                SoundDefOf.Mouseover_ButtonToggle
            );

            if (isActive != MaiyaLabelerMod.Settings.showRoomLabels)
            {
                MaiyaLabelerMod.Settings.showRoomLabels = isActive;
                MaiyaLabelerMod.Settings.showZoneLabels = isActive;
            }
        }
    }
    ...
    public class MaiyaLabelerMod : Mod
    {
        ...
        public override void DoSettingsWindowContents(Rect inRect)
        {
            ...
            // 传入类型 Key (TypeRoom/TypeGrow/TypeStock) 用于 DrawSettingRow 内部拼接
        ...
        private void DrawSettingRow(Listing_Standard list, string labelKey, ref bool iconToggle, ref Color color)
        {
            ...
            if (Widgets.ButtonInvisible(colorRect))
            {
                Find.WindowStack.Add(new Dialog_ChooseColor(
                    (newCol) =>
                    {
                        // 这里比较trick，因为传入的是 Key，我们需要判断 Key 来决定修改哪个设置
